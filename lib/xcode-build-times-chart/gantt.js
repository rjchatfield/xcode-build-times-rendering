...
function adgColor(taskName) {
    var str = taskName;
    let suf = (suffix) => str.endsWith(suffix)
    let isTest = str.endsWith("Tests");
    if (isTest) {
      str = str.substring(0, (str.length - "Tests".length));
    }
    let c = (letter) => colors[letter][isTest ? 75 : 300]

    const defaultColor = "#C1C7D0"; // N50

    if (str === "JIRA")    { return colors["N"][800] }
    if (str === "JIRAKit") { return colors["N"][800] }
    if (suf("Resources"))   { return defaultColor }
    if (suf("Assets"))      { return defaultColor }

    if (suf("Domain"))      { return c("G") }
    if (suf("Primitives"))  { return c("G") }
    if (suf("Data"))        { return c("R") }
    if (suf("Persistence")) { return c("R") }
    if (suf("Feature"))     { return c("B") }
    if (suf("UI"))          { return c("T") }
