...
function adgColor(taskName) {
    ...
    if (isTest) { str = str.substring(0, (str.length - "Tests".length)) }
    const c = (letter, num) => colors[letter][num] + (isTest ? "55" : "FF") // apply alpha if `isTest`
...
function build_gantt_chart() {
    ...
    const build_time_min = Math.round(build_time / 6) / 10
    ...
    const legend = [
        ['Build time', build_time_min + 'm (' + build_time + 's)'],
        ['Domain', 'green'],
        ['Data', 'red'],
        ['Feature', 'blue'],
        ['Jira & JIRAKit', 'N800'],
        ...
        const title = t[0];
        const value = t[1] + '';
        return title.padEnd(22) + ' : ' + value.padStart(12)
    }).map(function (row) {
